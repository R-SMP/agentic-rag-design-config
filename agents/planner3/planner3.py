"""The topology-3 hub — the Planner, doing the Orchestrator's job too.

Topology 3 is topology 5 with two MERGES: the DC Input Creator and the Tool
Caller become the **Design Engineer**, and the User Input Inspector and the
DC Output Inspector become the **Requirements Analyst**.  The hub is the
Planner in both, doing the same job — which is why this is a separate class
rather than topology 3 reusing ``Planner5``: a shared class would put the
LIVE 5-agent system at risk on every 3-agent edit.

**This file began as a byte-for-byte copy of
``agents/planner5/planner5.py``** and was then re-pointed — the same build
order that produced Planner5 from the Orchestrator, and for the same
reason: copying the LIVE hub means every fix made to it since is inherited
rather than re-derived.  The retired 3-agent Architect is NOT the ancestor;
it merged the UII into the hub, which this topology does not do.
...
Differences from Planner5, every one a consequence of the two merges:

* ``_agents_by_key`` holds FOUR entries rather than six -- the hub, the two
  merged agents and the Receptionist;
* the Planner has no edge to a Tool Caller, because there is none:
  generation is the Design Engineer's second half;
* the Design Engineer CAN hand back to the hub.  In topology 5 the Tool
  Caller is the one agent that cannot escalate, and merging it into the DC
  Input Creator -- which can -- hands that back.  It is the one capability
  the merge ADDS rather than removes;
* the precision-refine counter watches the REQUIREMENTS ANALYST.  Ported
  unchanged it would still read ``"dc_output_inspector"``, a key this
  topology never builds, so it would never fire, ``precision_rounds`` would
  stay 0 forever, and the entire section-matching loop would vanish with NO
  error and NO log line.  That is the quietest failure class this system
  has, and it has already happened once -- to standing directives, during
  the 5-agent build.

Everything else is inherited: the PLANNER's prompt and its four runtime
slots, ``_persist_plan`` / ``_save_plan_to_file`` so ``current_plan.txt``
keeps being written, no chain-log feed, and no ``PLANNER_FIRST`` /
``DC_INSPECTOR_ENABLED`` branching (``prompts`` forces both off for any hub
that is not the Orchestrator).

The dispatch contract is unchanged, so all three hubs stay drop-in
...
# a directive onto a hop back into its own author is pointless.
#
# This matters MORE here than in topology 5.  The hub sits OUTSIDE the refine
# loop, so it cannot re-issue a directive each round; the dispatcher applies
# this to EVERY hop, not only hub-outgoing ones, and that is what keeps a
# directive alive across Design Engineer <-> Requirements Analyst rounds the
# Planner never sees.
...
class Planner3(BaseChainAgent):
    """The topology-3 hub: plans, dispatches and approves.
    ...
    *, llm_cache) construction signature, the snapshot/restore
    ...
    def __init__(
        ...
        *,
        ...
        if session is None:
            raise TypeError(
                "Planner3 requires a Session.  Construct one via "
        ...
        # ``dc_inspector_enabled`` is deliberately absent: topology 3 has no
        ...
        # NO ``self.planner``: this agent IS the Planner.  The two merged
        # agents stand in for four of topology 5's.
        self.receptionist = Receptionist(
            state=_state_for("receptionist"), session=session,
        )
        self.design_engineer = DesignEngineer(
            state=_state_for("design_engineer"), session=session,
        )
        self.requirements_analyst = RequirementsAnalyst(
            state=_state_for("requirements_analyst"), session=session,
        )
        ...
        # Registry for the dispatch driver.  FOUR entries: this agent is its
        # own ``planner`` row, plus the two merged agents and the
        # Receptionist.  ``extra_utilities/hub_registry.py`` parses this
        # literal to answer "which agents does this hub BUILD" -- the
        # question the DH validates every schedule row's ``from_agent``
        # against, and an unresolvable one writes an ERROR: row into the R2
        # mirror and the Postgres chunks table.
        self._agents_by_key: dict = {
            self.AGENT_KEY:         self,
            "design_engineer":      self.design_engineer,
            "requirements_analyst": self.requirements_analyst,
    ...
    def _wire_routing(self) -> None:
        ...
        NO branches: topology 3 has no ``PLANNER_FIRST`` ordering to choose
        (the hub IS the planner) and no DC Input Inspector to switch in or
        out, so the edge set is fixed::

            Receptionist         -> Planner
            Planner (this hub)   -> Receptionist, Design Engineer,
                                    Requirements Analyst
            Design Engineer      -> Requirements Analyst, Planner
            Requirements Analyst -> Design Engineer, Planner

        Eight edges.  The Planner has NO edge to a Tool Caller because there
        is none -- generation is the Design Engineer's second half -- and,
        unlike topology 5's Tool Caller, the Design Engineer CAN hand back
        to the hub.

        The Design Engineer <-> Requirements Analyst pair is the refine loop,
        and the Planner is deliberately NOT in it: those two route back and
        forth directly, and the hub is reached at phase boundaries and on
        escalation.

        For BOTH merged agents, FORWARD and ESCALATE collapse into the SAME
        ...
        # Design Engineer -- FORWARD to the Requirements Analyst once it has
        # generated, and back to the hub to CLARIFY or to hand over a problem
        # it cannot resolve.  Both are the same tool.
        self.design_engineer.set_routing_tools([
            build_routing_tool("design_engineer", "requirements_analyst",
                               self.design_engineer, cl),
            build_routing_tool("design_engineer", self.AGENT_KEY,
                               self.design_engineer, cl),
        ])

        # Requirements Analyst -- the other half of the refine loop, plus the
        # return edge.  Its Design Engineer edge carries what took TWO edges
        # in topology 5: the DCOI's "change a parameter" hop to the DC Input
        # Creator AND its "re-render this attempt" hop to the Tool Caller,
        # which are now one agent.
        self.requirements_analyst.set_routing_tools([
            build_routing_tool("requirements_analyst", "design_engineer",
                               self.requirements_analyst, cl),
            build_routing_tool("requirements_analyst", self.AGENT_KEY,
                               self.requirements_analyst, cl),
        ])

        # This hub -- three routing edges, plus the PLANNER's own utility
        # tools.
        #
        ...
        hub_tools = [
            ...
            build_routing_tool(self.AGENT_KEY, "design_engineer", self, cl),
            build_routing_tool(self.AGENT_KEY, "requirements_analyst",
                               self, cl),
        ...
        routing_block = routing_instructions(
            agent_name="Planner",
            next_agent="Design Engineer",
            prev_agent="Requirements Analyst",
    ...
    def run(self, message: str) -> AgentHop:
        ...
        for _ in range(MAX_PLANNER3_STEPS):
            check_stop_or_raise()
    ...
    def dispatch(self, kickoff_message: str,
        ...
        for _ in range(MAX_DISPATCH_HOPS):
            ...
            if current == self.AGENT_KEY:
                ...
                orch_visits += 1
                if orch_visits > MAX_PLANNER3_VISITS:
            ...
            # A6 (precision sections): a refine ROUND is one hop into the
            # REQUIREMENTS ANALYST while a precision standing directive is
            # active -- NOT one hop.  A full cycle Requirements Analyst ->
            # Design Engineer -> Requirements Analyst increments this ONCE,
            # on the arrival, so MAX_SECTIONS_REFINE_ROUNDS means the same
            # number of render-and-judge cycles it means in every topology.
            #
            # The key had to change with the merge.  Left as
            ...
            if hop.target == "requirements_analyst" and self.session.standing_directives:
                precision_rounds += 1
    ...
    def _surface_limit_to_user(self, reason_label: str) -> str:
        ...
        ra_msg = _last_text_message(self.requirements_analyst)
        if ra_msg:
            summary_lines.append("Last Requirements Analyst verdict:")
            summary_lines.append(_truncate(ra_msg, 800))
            summary_lines.append("")

        de_msg = _last_text_message(self.design_engineer)
        if de_msg:
            summary_lines.append("Last Design Engineer report:")
            summary_lines.append(_truncate(de_msg, 800))
            summary_lines.append("")
    ...
    def reset(self) -> None:
        ...
        self.design_engineer.reset()
        self.requirements_analyst.reset()
    ...
    _AGENT_KEY_ALIASES: dict = {
        ...
        "deng": "design_engineer",
        "requirements analyst": "requirements_analyst",
        "requirements_analyst": "requirements_analyst",
        "ra": "requirements_analyst",
        # The merged-away names still resolve, onto whichever agent absorbed
        # them: a prompt, a log or a user carried over from topology 5 or 7
        # still gets an answer instead of "unknown agent".
        "dc input creator": "design_engineer",
        "dc_input_creator": "design_engineer",
        "dcic": "design_engineer",
        "tool caller": "design_engineer",
        "tool_caller": "design_engineer",
        "tc": "design_engineer",
        "user input inspector": "requirements_analyst",
        "user_input_inspector": "requirements_analyst",
        "uii": "requirements_analyst",
        "dc output inspector": "requirements_analyst",
        "dc_output_inspector": "requirements_analyst",
        "dcoi": "requirements_analyst",
    ...
    def run_feedback_round(
        ...
        *,
        ...
        # DC Input Inspector case to handle: _agents_by_key holds four keys
    ...
    def dump_histories(self, output_dir) -> list:
        ...
        agents = [
            ...
            ("design_engineer",      self.design_engineer,       getattr(self.design_engineer, "system_prompt", None)),
            ("requirements_analyst", self.requirements_analyst,  getattr(self.requirements_analyst, "system_prompt", None)),
        ]
