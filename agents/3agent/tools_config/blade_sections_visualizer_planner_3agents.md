...
The standard product of a design cycle is the full 3D propeller: the Design
Engineer points ``generate_and_render_propeller`` at an attempt's
...
the Requirements Analyst to judge.
...
the full 3D propeller.  The Design Engineer generates it from an attempt's
...
Make the render type explicit when you route — the chain should tell the Design
Engineer to render just the blade sections, or just the full 3D geometry, or
both.  When both are needed, take them ONE AT A TIME: get one type right
(generate → the RA judges → refine) before deciding whether the other is
still needed.
