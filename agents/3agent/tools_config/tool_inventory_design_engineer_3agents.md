...
<<BSV_ON>>2. **render_blade_sections** — render JUST the blade cross-sections
...
3. **generate_and_render_propeller**<</BSV_ON>><<BSV_OFF>>2. **generate_and_render_propeller**<</BSV_OFF>> — build
   ...
   warnings).
<<BSV_ON>>4.<</BSV_ON>><<BSV_OFF>>3.<</BSV_OFF>> **calculate** — evaluate arithmetic / boolean expressions.
<<BSV_ON>>5.<</BSV_ON>><<BSV_OFF>>4.<</BSV_OFF>> **read_attempts(attempt_numbers=None)** — with no argument, a
