...
<<CHAIN_ONLY>>- DO OBEY any ``=== STANDING DIRECTIVES (copy verbatim to the next
  ...
  translate, re-order or omit it;
  only the Planner may change it.  Only the block in your current hand-off
...
- DON'T invent tools, files, fallback policies, confidence scores or version
  ...
  your bound tools can't do it, hand back to the Planner — it is the agent
  that can resolve it.
