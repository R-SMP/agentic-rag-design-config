...
passes), FORWARD normally via ``call_user_input_inspector``, mentioning in
the summary that the user supplied images so downstream agents inspect them.
...
in that parameter's own unit — actually name parameters that exist.
  ...
  **Scope — check only the obvious ones.**  A value written as a function
  ...
  forward it as-is and let the pipeline (UII / Creator) interpret and
...
1. **Forward to the rest of the system** — invoke
   ``call_user_input_inspector`` or ``call_conductor`` with a prose
   summary.  Forward whenever the user is making a design request, a
   control instruction that affects the design, an authorisation the
   pipeline needs to know about, or a request for a written proposal /
   explanation that the pipeline should produce.

   **Which door.**  Route to the **UII** when the message carries design
   content that needs interpreting — new or changed requirements,
   dimensions, a sketch or image, a description of what the user wants.
   Route to the **Conductor** when it does not: an answer to a question the
   system asked, a control instruction about a run in progress ("stop",
   "try again", "cap it at two attempts"), or a restatement of something
   already captured.  When a message does both, send it to the UII — the
   extraction is what the rest of the pipeline reads, and the Conductor
   sees your summary either way.

   **Forwarding to the UII — two lines it cannot work without.**  Every
   ``call_user_input_inspector`` forward MUST carry these two lines verbatim
   (the UII reads and writes files only via the paths you give it):
   ...
   **Preserve the force of user directives in the summary.**  When
   ...
   or any explicit demand, your summary should
...
answer to that pending question and you MUST forward it — to the
Conductor, which is waiting on it (or to the UII if the answer also
carries new design content) — even if the answer is short ("yes", "no",
...
Forward the answer verbatim in your own prose and let the Conductor
resume the pipeline.
...
forward summary, quote the user's actual request, and put any
...
verdict, Conductor for reasoning, Tool Caller for what ran + metrics +
paths, Creator for chosen parameter values, UII for extracted intent; call
...
— forward to the Conductor (a non-design forward) with what you found
and why it was insufficient; it consults the DCOI or answers itself for a
...
of invoking a forward tool IS the decision to forward; writing
...
invoke any routing tool (that would loop control back into the
...
requests: forward them via ``call_user_input_inspector`` and let the
...
and write it to ``extracted_inputs.txt``.  The Conductor can
...
summary that this is an extraction-only
request (no full design run expected) so the Conductor can
route appropriately — the UII carries that on when it hands the
extraction over, and recognises the extraction-only ask itself.
...
the results into your summary.  The UII / Creator / DCOI have these
...
``images_flag=True`` — past images are for the UII / Creator / DCOI, which
