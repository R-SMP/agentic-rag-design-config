...
- **SOFT TARGET** — a value marked ``SOFT TARGET (goal: …; keep near … if
  ...
  it settles the parameter only when the goal does NOT bear on it.
- **FREE** — a parameter absent from QUANTITATIVE INPUTS: it is your choice
  within range.
...
authorisation, discoverable from EITHER of these:
  ...
  (B) the **extraction's DESIGN INTENT section** records one.
