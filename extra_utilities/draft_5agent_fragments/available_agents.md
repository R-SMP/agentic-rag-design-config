...
- **User Input Inspector (UII)** — the only agent that interprets raw user
  ...
  every new user message (the Receptionist routes to it) and then either
...
- **Tool Caller (TC)** — reads ``parameters.json`` and calls exactly two
  ...
  ``calculate`` (arithmetic only).  It REUSES an existing mesh / renders in
