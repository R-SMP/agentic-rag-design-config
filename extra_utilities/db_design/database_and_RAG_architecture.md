...
| T22 | **Use `sessions.user_id` column for multi-user identification.** | The column is reserved on schema v5 for a per-user identifier.  Phase 3C reads `self.session.user_id` and forwards it to `upsert_session`, but `Session.user_id` defaults to `None` and is never set by the dispatch layer today.  Direct quote from the design discussion: *"if we want to identify different users, we can use this column spot and modify slightly the frontend to collect this user ID information"*. | Frontend: capture user ID at session start (login flow, URL parameter, or a "who is using this?" header field).  Dispatch layer: store on `Session.user_id`.  `populate_database`'s existing `upsert_session(user_id=self.session.user_id, ...)` call then automatically includes it — no further DH code change.  See also F22 (deferred user-identifier work) and `warnings_developer.md` W27. |
...
### 9.5 Phase 3C — Hook DH save flow into Postgres  (NOT STARTED — design locked 2026-06-02)

> **Design locks (2026-06-02, Q-3C-1 through Q-3C-7):** the
> Phase 3C plan below reflects the answers to the Phase 3C design
> questions raised during the resume sprint.  Where this section
> diverges from the original v9 plan (commit `a7e6a7f`), the new
> design wins — the divergences are flagged inline.

**Tool rename (Q-3C-1).**  ``save_attempt_artefacts`` is renamed
to ``save_attempt_data`` to reflect its broader Phase 3C scope —
in addition to the R2 artefact upload, the tool now also performs
the Postgres-side per-attempt work (``upsert_attempt`` +
``upsert_attempt_parameters``).  The force-tool wiring, the DH
prompt, and developer-notes W18 are updated in lockstep.

**``save_attempt_data`` extended behaviour.**  For each attempt
the DH selects (via the force-tool turn on an identifying
attempt-related question), the tool performs five sub-steps:

1. Scan ``attempts/<NNN>/`` for ``*.obj`` → derive
   ``has_geometry`` (Q7: detection is automatic, NOT declared by
   the DH).
2. Scan ``attempts/<NNN>/`` for ``render_*.png`` → derive
   ``has_renders``.
3. Read ``attempts/<NNN>/parameters.json``.
4. Call ``db_writer.upsert_attempt(session_id, attempt_label,
   schema_version, parameters_json, has_geometry, has_renders)``
   → cache the returned BIGSERIAL ``attempt_id`` in
   ``attempt_id_by_nnn`` keyed by the 3-digit ``nnn`` (e.g.
   ``"001"``), so subsequent per-Q+A inserts can FK-reference it.
   NNN is used as the cache key rather than the full
   ``attempt_label`` because the per-Q+A integration sites in
   ``populate_database`` have ``nnn`` available via
   ``_normalise_attempt_input``, not the full folder name.
   Also call ``db_writer.upsert_attempt_parameters(
   attempt_id, parameters)`` with the long-format scalar dict.
5. Upload the 6 whitelisted artefact files to R2 (existing
   behaviour, unchanged).

**Failure independence (Q-3C-2 = A).**  The R2 upload and the
Postgres upserts are independent — either failing logs an ERROR
but the other still happens.  The tool returns success if R2
succeeded (preserving the original ``save_attempt_artefacts``
contract); Postgres-side failures surface only in the log.

**Only DH-chosen attempts are persisted (DIVERGENCE from earlier
v9 plan).**  The earlier plan said "at startup, walk
``config.ATTEMPTS_DIR`` and ``upsert_attempt`` every attempt
folder so chunks FKs resolve".  Q6 overrides this: attempts the
DH does NOT select via an identifying question are NOT persisted
in Postgres (no row in ``dc_attempts``, no rows in
``dc_attempt_parameters``, no attempt-scoped ``chunks`` rows).
FK resolution is guaranteed by the timing: the force-tool turn
runs BEFORE the attempt sub-rows, so the BIGSERIAL ``attempt_id``
is cached before any attempt-scoped ``insert_chunk`` fires.

**``upsert_session`` at populate_database start (Q-3C-3 = A).**
A single call to ``db_writer.upsert_session(...)`` at the very
start of ``populate_database()``, before any schedule processing.
Pre-creates the parent row that every chunks FK (directly) and
every dc_attempts FK (transitively) needs.

**Per-Q+A ``insert_chunk`` integration.**  After each
``_write_entry()`` (or ``_write_error_entry`` for error rows)
succeeds locally, call ``db_writer.insert_chunk(...)`` with the
same ``(session_id, agent_from, field, attempt_id, item_index,
field_type, question, body, is_error)`` data.  The local ``.txt``
write stays as the on-disk source of truth during Phase 3C;
Postgres becomes the retrieval surface in parallel.

**Item-index forcing for attempt-scoped single-pair rows
(Q-3C-E2 = A).**  When a row is attempt-scoped (the BIGSERIAL
``dc_attempts.attempt_id`` of a DH-resolved attempt) AND the DH
emitted a single QUESTION/ANSWER pair (``item_index=None`` on the
local ``.txt``), ``insert_chunk`` is called with ``item_index=1``
instead of ``None``.  This makes the chunks UNIQUE tuple
``(session_id, agent_from, field, attempt_id, item_index,
embedding_model)`` all-non-NULL so the constraint actually engages
— a re-run of ``populate_database`` for the same attempt-scoped
row then returns ``SKIPPED_UNIQUE`` rather than silently
double-writing.  Session-scoped single-pair rows keep
``item_index=None`` — the schema's NULL-distinct semantics are
intentional for those (see the chunks table NOTE in
``database_PostgreSQL_schema_v5.sql``).  See
``warnings_developer.md`` W28.

**Error rows in chunks (Q-3C-E1 / E6 = α).**  When the DH writes
an error placeholder via ``_write_error_entry`` (agent unknown
OR conversation raised), a corresponding ``chunks`` row is also
written: ``is_error=True``, ``field_type='Semantic'``,
``body=f"ERROR: {message}"``, with full stitch + embed.  The
embedding cost is wasteful for short error text but the partial
HNSW index already excludes ``is_error`` rows from retrieval, so
search performance is unaffected — the clean semantic of "every
Semantic row that is NOT empty has an embedding" is preserved.

**``insert_chunk`` outcome handling at the integration site
(Q-3C-4 = A).**

  - ``INSERTED``: log INFO, continue.
  - ``SKIPPED_UNIQUE``: log INFO, continue.
  - ``SAFETY``: log WARN, continue.  The DH's overall save still
    succeeds — the SAFETY return means data is preserved in the
    R2 safety folder, just not retrieval-indexed in Postgres.

**Cascade behaviour (Q-3C-5 = A).**  Implemented in Phase 3C,
not deferred.  Two trigger conditions:

  1. An identifying-Q ``insert_chunk`` returns ``SAFETY``.
  2. The ``upsert_attempt`` call inside ``save_attempt_data``
     fails for an attempt.

Either condition adds the attempt's NNN to a
``cascaded_attempt_nnns`` set in the populate_database call.
For every subsequent attempt-scoped Q+A on a cascaded attempt:

  - Skip ``insert_chunk`` entirely.
  - Call ``db_writer.save_to_safety_folder`` directly with
    ``cascade_source="identifying-Q for attempt_<NNN> failed
    (see <filename> in this folder)"``.

**R2 mirror stays active during Phase 3C.**  Belt-and-braces
while we verify Postgres writes work end-to-end.  Phase 3D drops
the ``.txt`` suffix from the R2 mirror suffix list.

**End-Session feedback wiring (Q-3C-6 = A).**  ``web_app.py``'s
``/api/end`` HTTP handler is the direct caller of
``db_writer.save_session_feedback(...)`` — no LLM in the loop.
The handler maps the modal payload to the
``FIXED_FEEDBACK_QUESTIONS`` ids and passes the satisfaction +
answers dict.
