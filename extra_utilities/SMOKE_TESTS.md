...
| `smoke_test_async_turn.py` | full-runtime |  | Asserts the /api/turn 202 + SSE turn_done contract with dispatch_turn stubbed: 202 with a 12-hex turn_id, 400 on a blank message, 409 on a concurrent turn, an end-to-end SSE turn_done payload (turn... |
...
| `smoke_test_dh_force_tool_buffer.py` | none |  | Regression guard for the ID1484 save that wrote zero interview answers: lifts the shipped _force_tool_args text out of database_handler.py and drives it against stubs, asserting every forced tool call it emits into self.messages is answered by a ToolMessage.  An orphan fails every later DH call under /v1/responses. |
...
| `smoke_test_llm_client_cache.py` | llm-libs |  | Asserts the LLM client cache memoises on the (provider, model, api_key) triple: same agent hits, two agents with the same triple share one client, a different triple or a different api_key builds a... |
...
| `smoke_test_no_parallel_kwarg.py` | llm-libs |  | Constructs the whole Orchestrator + 7-chain-agent stack against a fake Anthropic client and asserts every construction-time bind_tools() call receives ZERO kwargs (i.e. the parallel_tool_calls=Fals... |
...
| `smoke_test_param_rename.py` | 3d-stack |  | Intercepts EvaluateDefinition to capture the ParamName of every input tree the tool sends to RhinoCompute and asserts exactly 17 names arrive — the 16 canonical camelCase inputs from parameter_keys... |
...
| `smoke_test_prompt_tool_audit.py` | none | regenerates `prompt_pdf/dump*.json` if absent | Fails when an assembled prompt names a tool that NO agent in that topology binds - the shape of every prompt/tool drift on record (the Conductor's `read_user_queries`, the DCOI's `list_input_files`, a topology-5 prompt still saying `call_orchestrator`).  Both sides derived, never transcribed: prompts from `topology_prompt_snapshot`, bound tools from dump.json plus the routing edges read out of `planner5.py`.  Topology 3 cannot be derived that way - its two merged agents are NEW classes with no 7-agent twin - so its bound set is taken by CONSTRUCTING the real hub in a child process and recording what each agent is handed. |
| `dry_run_topology.py` | none | `--topology N`; one subprocess per topology | Drives ONE COMPLETE TURN per topology against a scripted fake LLM - no network, no tokens.  Everything but `invoke_with_retry` is the production path: the real `dispatch_turn`, the real Receptionist `validate_input`, every agent's real run loop, the real routing tools and the real hub dispatch loop.  Asserts the exact hop SEQUENCE, so a mis-route fails on the spot naming who was invoked and who should have been, and proves each edge is really bound (an unbound tool name stalls the turn).  This is the only guard that catches the class of defect both live 5-agent runs found and the whole static suite missed - the blank user reply, the discarded start target, the AttributeError on an agent the hub never builds.  Mutation-tested: deleting a refine edge fails it, and re-introducing the historic hard-coded `"orchestrator"` forward test fails topology 3 while topology 7 still passes - which is exactly why that bug shipped. |
| `smoke_test_topology3_tool_text.py` | none | | Proves the topology-3 tool overlay says what it CLAIMS to say, in the shape each claim is made: values marked verbatim are byte-identical to their topology-5 source; the one superset-wins copy really does CONTAIN the other parent's text; roster-derived descriptions name every agent the hub builds and no agent it does not, checked against `planner3.py`'s own `_agents_by_key`; and the one genuine merge is still absent, so the resolver returns exactly what it returned with no overlay.  Also checks the overlay is REACHED -- a module written but never registered in `_OVERLAY_MODULE_BY_TOPOLOGY` is silently inert.  Mutation-tested on both: drifting a verbatim value by one word, and un-registering the module. |
| `smoke_test_topology_fragments.py` | none | py>=3.10 - **writes into the real tree** | Builds every prompt for topologies 7, 5 and 3 under both PLANNER_FIRST settings and asserts NO-LEAK, ISOLATION, no unsubstituted $slots, correct $routing_hub resolution, the CHAIN_ONLY hub filter, clean degradation for an unregistered topology, and - for a fully forked topology - MIRROR: that it reads NOTHING from the shared prompt trees at all.  Findings that await an owner-approved prompt edit are listed as KNOWN-PENDING rather than silently passed. |

## Root utilities (not tests)  (3)

| Script | Needs | Flags | Guards / does |
|---|---|---|---|
| `check_mesh_components.py` | 3d-stack |  | Standalone .obj diagnostic: parses a mesh group-by-group and reports verts/faces/watertightness/degenerate-face count/signed volume for each component and for the concatenated whole, both as-writte... |
