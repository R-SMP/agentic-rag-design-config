...
prefer ``retrieve_user_inputs(...)`` (how past users' inputs looked + how
the chain read them) and ``retrieve_attempt(...)`` (past renders + their
verdicts): past renders show what passed visual inspection and what failed,
and why.  Both download to a local folder and list it; pass a listed path to
``view_images`` — with
